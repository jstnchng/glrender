...
{ 
    ...
    if (checkerboard > 0) {
        float width = 4.0 / float(checkerboard);
        float cx = floor( (pos.x) / width);
        float cy = floor( (pos.y) / width);
        float cz = floor( (pos.z) / width);

        float xyz = (cx + cy + cz);
        float res = xyz - 2.0 * floor(xyz/2.0);
        if (res == 0.0){
            material_diffuse = vec4(1.0, 1.0, 1.0, 0.0);
        }  
        else{
            material_diffuse = vec4(0.0, 0.0, 0.0, 0.0);
        }
    } 
